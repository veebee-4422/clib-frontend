...
export function SideBaRec({ image, title, synopsis, tags }: SideBarRecProps) {
    const darkMode = true;
    return (
        <div className="w-full mb-4 bg-white border border-gray-200 rounded-lg hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700">
            ...
            <div className="flex flex-col justify-between p-4 leading-normal">
                ...
                {tags.length && <ul className="grid xl:grid-cols-3 lg:grid-cols-2 grid-cols-1 gap-2 text-gray-900 dark:text-white">
                    {tags.map((tag) => {
                        return (<li className="flex flex-row items-center">
                            <img className="h-4 mx-1" src={darkMode ? tagDark : tagLight} alt="tag" />
                            <p className="hover:underline md:me-6 ">{tag}</p>
                        </li>)
